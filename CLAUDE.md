...
- `Post` + `Category` + `Tag` — the blog/CMS. Index is `/blog`; individual posts are at `/{year}/{month}/{slug}` (no `/blog` prefix), WordPress-style — **`slug` is intentionally NOT unique in the database**; uniqueness comes from the year/month/slug combination (see `BlogController::show` and `Post::getUrlAttribute()`). SEO fields (`meta_title`, `meta_description`, `og_image`) fall back to `title`/`excerpt`/`featured_image` via the `seo_title`/`seo_description`/`seo_image` accessors on the model.
